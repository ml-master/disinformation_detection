...
# 假新闻数据加载器，用于对假新闻数据集进行评估，只读取fake目录下的新闻数据
class FakeNewsNetWithEntityFake(Dataset):
    def __init__(self, data_path, corpus) -> None:
        super().__init__()
    ...
    def read_data(self, data_path, corpus):  # TODO tokenize
        max_len = 512
        labels, texts, seq_lens, weights, entities = [], [], [], [], []
        for i, data_file in data_file_gen_fake(data_path, corpus):
            label = data_file.split('/')[-2]
            label = label2id.get(label, -1)

            # text为新闻文章内容，entity_seq为其对应的实体序列（以空格隔开）
            text, entity_seq = read_text_with_entity(data_file)  # type: ignore
            if text is None or len(text.split()) == 0:
                continue
            text_split = text.split()
...
# 真新闻数据加载器，用于对真新闻数据集进行评估，只读取real目录下的新闻数据
class FakeNewsNetWithEntityReal(Dataset):
    def __init__(self, data_path, corpus) -> None:
        super().__init__()
    ...
    def read_data(self, data_path, corpus):  # TODO tokenize
        max_len = 512
        labels, texts, seq_lens, weights, entities = [], [], [], [], []
        for i, data_file in data_file_gen_real(data_path, corpus):
            label = data_file.split('/')[-2]
            label = label2id.get(label, -1)
...
def record_files_gen(data_path):
    ...
    for label in os.listdir(data_path):
        ...
        for record in os.listdir(records_path):
            record_file = records_path + "/" + record + "/news_article.json"
            yield label, record_file

# 加载fake目录下的所有假新闻数据
def data_file_gen_fake(data_path, corpus=None):
    """
    -gossipcop
    -politifact
        -fake
...
def data_file_gen_real(data_path, corpus=None):
    """
    -gossipcop
    -politifact
        -fake
