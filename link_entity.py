...
# your tagme token
tagme.GCUBE_TOKEN = "64392264-df94-49c1-a1d3-3952e42d9917-843339462"
...
def extract_entities(data_path, client, break_point=None, stop=None):
    ...
    for data in record_files_gen(data_path):
        ...
        with open(f'.dataset/processed_data/gossipcop/{label}/{cnt}.txt', 'w+', encoding='utf-8') as f:
...
def write_entity(entity, label, cnt, corpus='gossipcop'):
    if entity is not None:
        with open(f'./processed_data/{corpus}/{label}/{cnt}.txt', 'a+', encoding='utf-8') as f:
            f.write(str(entity) + '\n')
...
def extract_entities_wo_neighbours(data_path, corpus, break_point=None):
    ...
    for data in record_files_gen(data_path):
        ...
        with open(f'./processed_data/{corpus}/{label}/{cnt}.txt', 'w+', encoding='utf-8') as f:
            f.write(text.replace('\n', ' ') + '\n') 
